...
from src.schemas.photos import CommentBase, CommentModel, CommentUpdate
from src.repository import comments as repository_comments
from src.services.auth import auth_service

router = APIRouter(prefix="/comments", tags=["comments"])

@router.post("/create/{photo_id}", response_model=CommentModel, status_code=status.HTTP_201_CREATED)
async def create_comment(
    ...
    return await repository_comments.create_comment(db, comment_data, current_user.id, photo_id)
...
async def edit_comment(
    ...
    if comment.user_id != current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You are not allowed to edit this comment")
