...
async def create_tag(
    body: TagBase, db: Session = Depends(get_db), current_user: str = "Some_User"
):
...
async def read_tag_by_id(
    tag_id: int, db: Session = Depends(get_db), current_user: str = "Some_User"
):
    tag = await repository_tags.get_my_tags(tag_id, current_user, db)
    if tag is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="404_TAG_NOT_FOUND"
        )
    return tag
